...
const LessonCardTeacher = (props) => {
  ...
  const generateQuiz = (_id) => {
    setShowAlert(true); 
    axios.post(`${process.env.REACT_APP_API_URL}/quizes/` + _id, null, {
      headers: {
        Authorization: `Bearer ${Auth.token}`
      }
    })
    .then((resp) =>{
    setLessons({...lesson,success:"Generated"})
    })
    .catch((err)=>{
      console.log(err.message)
      setLessons({...lesson, err: err.response.data.error.message })
    })
  }
